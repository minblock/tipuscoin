...
const char * const BITCOIN_CONF_FILENAME = "tipuscoin.conf";
...
#else
    const char* pszModule = "tipuscoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Tipuscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Tipuscoin";
#else
    // Unix
    return pathRet / ".tipuscoin";
#endif
#endif
}
